...
void CollectLivenessObjects(void **objects, int count, void *opaque) {
  ...
  for (int index = 0;
       index < count && collector->count < collector->objects.size(); ++index) {
...
UiProbeResult ProbeUnityUi(const Il2CppDynamicProbe &probe,
                           ...
                           const void *uiModuleImage, int screenHeight) {
  ...
  result.diagnosticStage = 60;
  stopGcWorld();
  void *livenessState = livenessAllocate(buttonClass, 0, CollectLivenessObjects,
...
int32_t ObserverMainThreadTick(uint64_t requestGeneration,
  ...
  if (snapshot.mainThread != 0) {
    const UiProbeResult ui =
        ProbeUnityUi(probe, coreImage, uiImage, uiModuleImage, height);
